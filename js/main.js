...
const DIPLOMAS = [
  ...
  { img: 'images/diploma_2/file-001.png', title: 'Диплом о профессиональной переподготовке — Московский институт психологии, психолог-консультант, 1800 часов, 2025' },
  { img: 'images/diploma_2/file-002.png', title: 'Приложение к диплому — Московский институт психологии' },
  ...
  { img: 'images/diploma_3/file-001.png', title: 'Приложение к диплому — Институт прикладной психологии, психолог-тренер' },
  { img: 'images/diplomas/diploma_4.png', title: 'Сертификат — Психотерапия взросления: введение и воссоединение с чувствами, 2024' },
  { img: 'images/diplomas/diploma_5.png', title: 'Сертификат — Эмоциональный интеллект в психотерапии взросления и терапия, центрированная на чувствах, 2024' },
];
...
let lbImg = null;
let diplomaAutoplayTimer = null;

function startDiplomaAutoplay() {
  if (!diplomaScrollEl || DIPLOMAS.length <= 1) return;
  stopDiplomaAutoplay();
  diplomaAutoplayTimer = setInterval(() => {
    const maxScroll = diplomaScrollEl.scrollWidth - diplomaScrollEl.clientWidth;
    if (diplomaScrollEl.scrollLeft >= maxScroll - 5) {
      diplomaScrollEl.scrollTo({ left: 0, behavior: 'smooth' });
    } else {
...
function stopDiplomaAutoplay() {
  if (diplomaAutoplayTimer) {
    clearInterval(diplomaAutoplayTimer);
    diplomaAutoplayTimer = null;
  }
}
...
async function loadSchedule(serviceId = '') {
  try {
    ...
    const response = await fetch(`${BACKEND_URL}/api/schedule?${params.toString()}`);
